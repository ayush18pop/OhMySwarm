 ...
 */
function toWireMessages(messages: LLMMessage[], systemPrompt?: string): unknown[] {
  const out: unknown[] = []
...
function parseRetryAfterMs(res: Response, errText: string, attempt: number): number {
  const header = res.headers.get('retry-after')
  if (header) {
    const seconds = Number(header)
    if (!Number.isNaN(seconds) && seconds > 0) return Math.ceil(seconds * 1000) + 250
  }
...
async function apiPost(body: object, retries = 4, attempt = 0): Promise<Response> {
  ...
  if (res.status === 429 && retries > 0) {
    ...
    console.warn(`[llm] 429 rate-limited — waiting ${waitMs}ms then retrying (${retries} left)`)
...
export async function summarizeMessages(
  ...
  const transcript = messages
    .map(m => {
      const contentStr = typeof m.content === 'string' ? m.content : JSON.stringify(m.tool_calls ?? '')
